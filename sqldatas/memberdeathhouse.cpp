...
MemberDeathHouse::MemberDeathHouse(QObject *parent)
    : CoolQ::SqliteService(*new MemberDeathHousePrivate(), parent)
{
...
CoolQ::SqliteService::Result MemberDeathHouse::addMember(qint64 gid, qint64 uid)
{
    Q_D(MemberDeathHouse);
...
CoolQ::SqliteService::Result MemberDeathHouse::removeMember(qint64 gid, qint64 uid)
{
    Q_D(MemberDeathHouse);
    QWriteLocker locker(&d->guard);
...
QHash<CoolQ::Member, qint64> MemberDeathHouse::members() const
{
    Q_D(const MemberDeathHouse);
    return d->members;
}

void MemberDeathHouse::expiredMembers(CoolQ::MemberList &members)
{
    Q_D(MemberDeathHouse);
    QWriteLocker locker(&d->guard);
